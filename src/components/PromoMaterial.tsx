...
export function PromoMaterialSelector({ materials, selectedId, onSelect }: PromoMaterialSelectorProps) {
  return (
    <div className="mobile-scroll-x -mx-4 overflow-x-auto px-4 pb-1 sm:mx-0 sm:overflow-visible sm:px-0 sm:pb-0">
      <div className="flex w-max min-w-full gap-2 sm:w-auto sm:grid sm:grid-cols-3 sm:gap-3">
        {materials.map((material, i) => {
          ...
          return (
            <motion.button
              ...
              className={`w-[min(260px,calc(100vw-3rem))] shrink-0 rounded-xl border p-3.5 text-left transition-all active:scale-[0.98] sm:w-auto sm:p-4 ${
                isSelected
        ...
        <div
          className={`overflow-hidden rounded-xl shadow-elevated ${
            isNotification || preview.type === 'app'
              ? 'w-full max-w-[200px] sm:max-w-[280px]'
              : preview.type === 'flyer'
                ? 'w-full max-w-[220px] sm:max-w-[320px]'
      ...
      >
        <div className="mb-4 flex flex-col gap-3 sm:mb-5 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h4 className="text-sm font-bold text-sk-gray-800 sm:text-base">{material.reportHeading}</h4>
            <p className="mt-0.5 text-xs text-sk-gray-500 sm:text-sm">{material.description}</p>
          ...
          <div className="mobile-scroll-x -mx-1 flex gap-2 overflow-x-auto px-1 pb-0.5 sm:mx-0 sm:flex-wrap sm:overflow-visible sm:px-0 sm:pb-0">
            ...
            {clickRate && (
              <span className="flex shrink-0 items-center gap-1.5 rounded-full bg-sk-orange-light px-3 py-1.5 text-xs font-semibold text-sk-orange">
                클릭율 {clickRate}%
              </span>
            )}
          </div>
