...
function computeMarketingAggregates() {
...
function buildPushDataTable(material: PromoMaterial): ReportDataTable {
  ...
  return {
    columns: [
      { key: 'media', label: 'Media 구분', align: 'left', width: '16%' },
      { key: 'date', label: '발송일자', align: 'center', width: '14%' },
      { key: 'sent', label: '발송건수', align: 'right', width: '16%' },
      { key: 'reach', label: 'Noti', align: 'right', width: '16%' },
      { key: 'clicks', label: '클릭수', align: 'right', width: '16%' },
      { key: 'rate', label: '클릭율', align: 'right', width: '12%' },
    ],
    rows: rows.map((r) => ({
      media: r.media,
      date: r.sendDate,
...
function buildBannerDataTable(material: PromoMaterial): ReportDataTable {
  ...
  return {
    columns: [
      { key: 'media', label: 'Media 구분', align: 'left', width: '12%' },
      { key: 'period', label: '노출 일자', align: 'center', width: '14%' },
      { key: 'placement', label: '배너 구분', align: 'left', width: '22%' },
      { key: 'pv', label: '노출 PV', align: 'right', width: '16%' },
      { key: 'clicks', label: 'PV클릭', align: 'right', width: '16%' },
      { key: 'rate', label: 'PV클릭율', align: 'right', width: '12%' },
    ],
    rows: rows.map((r) => ({
      media: r.media,
      period: r.period,
...
function buildMarketingSummaryDataTable(mkt: ReturnType<typeof computeMarketingAggregates>): ReportDataTable {
  const pushRate = mkt.clickRate.toFixed(2);
  const bannerRate = mkt.impressions > 0 ? ((mkt.bannerClicks / mkt.impressions) * 100).toFixed(2) : '0.00';
  ...
  return {
    columns: [
      { key: 'channel', label: '구분', align: 'left', width: '22%' },
      { key: 'reach', label: '도달/노출', align: 'right', width: '22%' },
      { key: 'clicks', label: '클릭', align: 'right', width: '20%' },
      { key: 'rate', label: '클릭율', align: 'right', width: '14%' },
    ],
    rows: [
      {
        channel: '타겟 Push',
...
function materialDataTable(material: PromoMaterial): ReportDataTable | undefined {
...
export function buildPerformanceReportSlides(): PerformanceReportSlide[] {
  ...
  promoMaterials.forEach((material) => {
    ...
    slides.push({
      ...
      dataTable: materialDataTable(material),
